...
namespace djinni_sqlite {
    ...
    std::shared_ptr<MyClientInterface> s_logger;
  
    std::shared_ptr<TodoList> TodoList::create_with_path(const std::string & path) {
        if (s_logger){
            s_logger->log_string("create_with_path " + path);
        }
        return std::make_shared<TodoListImpl>(path);
    }
    
    void TodoList::set_logger(const std::shared_ptr<MyClientInterface> &logger){
        s_logger = logger;
    }
